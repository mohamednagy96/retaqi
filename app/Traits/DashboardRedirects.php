...
trait DashboardRedirects{
    ...
    public function redirectRouteWithSuccessStore($route = null, $params = null){
        ...
        $redirect = redirect()->back()->with('success', __('تم حفط البيانات'));

        $route = $route ? $route : $this->redirectRoute();

        if($route){
            $redirect = redirect()->route($route, $params)->with('success', __('تم حفط البيانات'));
        }
    ...
    public function redirectRouteWithSuccessUpdate($route = null, $params = null){
        ...
        $redirect = redirect()->back()->with('success', __('تم تحديث البيانات بنجاح'));

        $route = $route ? $route : $this->redirectRoute();

        if($route){
            $redirect = redirect()->route($route, $params)->with('success', __('تم تحديث البيانات بنجاح '));
        }
    ...
    public function redirectRouteWithSuccessDelete($route = null, $params = null){
        ...
        $redirect = redirect()->back()->with('success', __('تم حذف البيانات'));

        $route = $route ? $route : $this->redirectRoute();

        if($route){
            $redirect = redirect()->route($route, $params)->with('success', __('تم حذف البيانات'));
        }
